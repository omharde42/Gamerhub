...
export class PostService {
  ...
  async list(params: { page?: number; limit?: number; hashtag?: string; userId?: string; following?: string }) {
    ...
    if (following) { const follows = await prisma.follow.findMany({ where: { followerId: following }, select: { followingId: true } }); where.userId = { in: [...follows.map(f => f.followingId), following] }; }
  ...
  async delete(id: string, userId: string) { const post = await prisma.post.findUnique({ where: { id } }); if (!post) throw new Error('Post not found'); if (post.userId !== userId) throw new Error('Unauthorized'); await prisma.post.delete({ where: { id } }); }
