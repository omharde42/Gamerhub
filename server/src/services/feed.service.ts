...
export class FeedService {
  async getFeed(userId: string, page: number = 1, limit: number = 20) {
    ...
    const followingIds = [userId, ...following.map((f: any) => f.followingId)];
    ...
    return { data: posts.map((post: any) => ({ ...post, isLiked: post.likes.length > 0, likes: undefined })), meta: { page, limit, total, totalPages: Math.ceil(total / limit), hasNext: page * limit < total, hasPrev: page > 1 } };
  }
