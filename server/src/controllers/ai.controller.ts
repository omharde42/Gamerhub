...
export class AIController {
  ...
  async chat(req: AuthRequest, res: Response, next: NextFunction) { try { const { message, history } = req.body; const profile = await prisma.profile.findUnique({ where: { userId: req.user!.userId } }); const response = await aiService.chat(message, history || [], profile); res.json({ success: true, data: { response } }); } catch (error) { next(error); } }
  async summarizeNews(req: AuthRequest, res: Response, next: NextFunction) { try { const { articles } = req.body; const summary = await aiService.summarizeNews(articles || []); res.json({ success: true, data: { summary } }); } catch (error) { next(error); } }
}
