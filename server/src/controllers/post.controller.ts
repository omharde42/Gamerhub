...
export class PostController {
  async create(req: AuthRequest, res: Response, next: NextFunction) { try { const post = await postService.create(req.body, req.user!.userId); io.emit('post:new', post); res.status(201).json({ success: true, data: post }); } catch (error) { next(error); } }
