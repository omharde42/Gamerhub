...
export function SearchOverlay({ isOpen, onClose, isStandalonePage = false }: SearchOverlayProps) {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape' && isOpen && !isStandalonePage) {
        onClose();
      }
    };
  ...
  const { data: recommendedGamers = [], isLoading: recommendedLoading } = useQuery({
    ...
    enabled: isOpen && recentSearches.length === 0 && query.trim() === '',
  });
  ...
  const highlightMatch = (text: string, search: string) => {
    ...
    return parts.map((part, i) =>
      part.toLowerCase() === search.toLowerCase() ? (
        <span key={i} className="text-emerald-400 font-extrabold bg-emerald-500/15 px-0.5 rounded">
          {part}
        </span>
      ) : (
        part
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className="fixed inset-0 z-50 flex flex-col bg-[#05070E] text-white w-full h-full overflow-hidden safe-area-all"
      >
        {/* 1. Fixed Search Header Bar */}
        <div className="h-16 border-b border-border/40 flex items-center px-3 sm:px-4 gap-2.5 bg-card/60 shrink-0 backdrop-blur-xl z-10">
          <Button
            variant="ghost"
            size="icon"
            onClick={handleBack}
          ...
          <div className="flex-1 relative flex items-center">
            ...
            <Input
              ...
              className="h-10 pl-10 pr-10 w-full bg-muted/40 border-0 rounded-full text-sm focus-visible:ring-1 focus-visible:ring-emerald-500/50 focus-visible:bg-muted/70 text-foreground placeholder:text-muted-foreground/70"
              variant="ghost"
              autoComplete="off"
              autoCorrect="off"
              autoCapitalize="off"
          ...
          <div className="px-4 py-2.5 shrink-0 border-b border-border/20 bg-card/20 z-10">
            <Tabs value={activeTab} onValueChange={(val) => setActiveTab(val)} className="w-full">
              <TabsList className="bg-muted/20 p-1 rounded-xl flex w-full justify-start gap-1">
                ...
                <TabsTrigger value="players" className="flex-1 rounded-lg text-xs font-bold data-[state=active]:bg-emerald-500 data-[state=active]:text-black">
                  👤 Players
                </TabsTrigger>
                <TabsTrigger value="teams" className="flex-1 rounded-lg text-xs font-bold data-[state=active]:bg-emerald-500 data-[state=active]:text-black">
                  👥 Teams
                </TabsTrigger>
        ...
        <div className="flex-1 overflow-y-auto p-4 max-w-3xl mx-auto w-full space-y-6">
          ...
          {query.trim().length === 0 && (
            <div className="space-y-6">
              ...
              {recentSearches.length > 0 ? (
                <div className="space-y-3">
                  <div className="flex items-center justify-between px-1">
                    ...
                    <button
                      ...
                      className="text-xs font-bold text-emerald-400 hover:underline transition-colors"
                    >
                      Clear All
                    </button>
                  </div>
                    ...
                    {recentSearches.map((term, idx) => (
                      ...
                      >
                        <div className="flex items-center gap-3 min-w-0">
                          <Clock className="h-4 w-4 text-muted-foreground group-hover:text-emerald-400 transition-colors shrink-0" />
                ...
                /* If HISTORY is EMPTY -> SHOW POPULAR TOPICS + SUGGESTED GAMERS */
                <div className="space-y-6">
                  {/* Popular Games Tags */}
                  <div className="space-y-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {['Clash of Clans', 'PUBG / BGMI', 'Valorant', 'CS2', 'Apex Legends', 'League of Legends'].map((game) => (
                        <Badge
                          ...
                          className="px-3.5 py-2 cursor-pointer bg-card/40 hover:bg-emerald-500/20 border-white/10 text-xs font-bold gap-1.5 rounded-2xl transition-all"
                        >
                          ...
                          >
                            ...
                            >
                              ...
                              <div className="min-w-0">
                                <p className="text-sm font-semibold truncate hover:text-emerald-400 transition-colors text-foreground">
                            ...
                            <Button
                              ...
                              className="h-8 text-xs font-bold rounded-xl gap-1 shrink-0 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/20 ml-2"
                            >
          ...
          {query.trim().length > 0 && !loading && searchError && (
            <div className="flex flex-col items-center justify-center py-16 text-center space-y-3">
              ...
              <div>
                <p className="text-sm font-extrabold text-foreground">{searchError}</p>
                <p className="text-xs text-muted-foreground mt-1">Please check your connection and try again.</p>
              </div>
              <Button variant="outline" size="sm" onClick={() => setQuery(query)} className="text-xs font-bold rounded-xl mt-2 border-emerald-500/40 text-emerald-400">
                Retry Search
              </Button>
            </div>
          )}
                  ...
                  if (item._type === 'player') {
                    ...
                    return (
                      ...
                      >
                        ...
                        <Badge variant="outline" className="text-xs font-bold gap-1 rounded-xl bg-emerald-500/10 text-emerald-400 border-emerald-500/20">
