...
import { Swords, Trophy, Target, CheckCircle2, AlertCircle } from 'lucide-react';
  ...
  if (!gameUid) {
    return (
      <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.3 }}>
        <Card variant="glass" className="border-amber-500/30 bg-gradient-to-br from-[#1F1708] via-[#0F0D06] to-black">
          <CardContent className="p-6 space-y-5">
            <GameConnectForm gameKey="bgmi" fieldName="uid" placeholder="BGMI Character ID (e.g. 1234567890)" gameLabel="BGMI" accent="amber" />
          </CardContent>
        </Card>
      </motion.div>
    );
    ...
    return (
      <div className="p-5 rounded-2xl bg-red-500/10 border border-red-500/30 flex items-center justify-between gap-4">
        <div className="flex items-center gap-2.5 text-red-300 text-xs font-semibold">
          ...
          <span>{(error as any)?.response?.data?.message || 'Unable to fetch BGMI data right now.'}</span>
        </div>
