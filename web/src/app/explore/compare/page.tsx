...
export default function ComparePage() {
  ...
  // 1. Fetch Common Connected Games (Intersection)
  const { data: games = [], isLoading: isLoadingGames } = useQuery({
    queryKey: ['compare-common-games'],
    queryFn: async () => {
      const res = await api.get('/compare/games');
  ...
  // 2. Fetch Friends Leaderboard for selected game
      ...
      {games.length === 0 ? (
        <Card variant="glass" className="border-amber-500/30 bg-gradient-to-br from-[#1F1708] via-[#0F0D06] to-black text-center p-8 sm:p-12 space-y-4">
          ...
          <div className="space-y-1 max-w-md mx-auto">
            <h3 className="text-lg font-extrabold text-white">No common games yet</h3>
            <p className="text-xs text-gray-400 leading-relaxed">
              Connect a game that your accepted friends also play to start competing on the live friends leaderboard.
            </p>
          </div>
        ...
        <>
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none">
            {games.map((g: any) => {
              ...
              return (
                <button
                  key={g.id}
                  onClick={() => { setSelectedGameId(g.id); setSelectedFriendId(null); }}
          ...
          {isLoadingLeaderboard ? (
            <div className="p-8 rounded-3xl bg-white/5 border border-white/10 animate-pulse space-y-4">
            ...
            <div className="space-y-6">
              ...
              <div className="flex flex-wrap items-center justify-between gap-4 p-5 rounded-3xl bg-gradient-to-r from-amber-950/60 via-black/80 to-purple-950/40 border border-amber-500/30">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-sm font-extrabold text-white">Your Friends Leaderboard</h3>
