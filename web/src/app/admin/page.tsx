...
export default function AdminPage() {
  ...
  return (<div className="space-y-6"><div className="flex items-center justify-between"><h1 className="text-2xl font-bold"><Shield className="h-6 w-6 inline mr-2 text-gaming-purple" />Admin Panel</h1><Badge variant="premium">Admin</Badge></div>
    ...
    <Tabs defaultValue="users"><TabsList><TabsTrigger value="users"><Users className="h-4 w-4 mr-1" />Users</TabsTrigger><TabsTrigger value="reports"><AlertTriangle className="h-4 w-4 mr-1" />Reports</TabsTrigger></TabsList>
      <TabsContent value="users"><Card className="glass-card"><CardContent className="p-6"><div className="space-y-3">{users?.data?.map((u: any, i: number) => (<div key={i} className="flex items-center justify-between p-3 rounded-lg border border-border"><div className="flex items-center gap-3"><Avatar className="h-10 w-10"><AvatarImage src={u.profile?.avatar || ''} /><AvatarFallback>{u.profile?.username?.charAt(0) || 'U'}</AvatarFallback></Avatar><div><p className="text-sm font-medium">{u.profile?.username || u.email}</p><p className="text-xs text-muted-foreground">{u.email} • {u.role}</p></div></div><div className="flex gap-2">{u.banned ? <Badge variant="destructive">Banned</Badge> : <Button variant="ghost" size="sm" className="text-destructive" onClick={() => banUser.mutate(u.id)}><Ban className="h-3 w-3 mr-1" />Ban</Button>}</div></div>))}</div></CardContent></Card></TabsContent>
      <TabsContent value="reports"><Card className="glass-card"><CardContent className="p-6"><div className="space-y-3">{reports?.data?.map((r: any, i: number) => (<div key={i} className="flex items-center justify-between p-3 rounded-lg border border-border"><div><p className="text-sm font-medium">{r.reason}</p><p className="text-xs text-muted-foreground">{r.description} • {formatDate(r.createdAt)}</p></div><Badge variant={r.status === 'PENDING' ? 'warning' : 'success'}>{r.status}</Badge></div>))}</div></CardContent></Card></TabsContent>
    </Tabs>
  </div>);
}
