...
export default function AdminPage() {
  ...
  const statCards = [{ label: 'Users', value: stats?.users || 0, icon: Users, color: 'text-blue-500' }, { label: 'Tournaments', value: stats?.tournaments || 0, icon: Trophy, color: 'text-yellow-500' }, { label: 'Jobs', value: stats?.jobs || 0, icon: Briefcase, color: 'text-green-500' }, { label: 'Organizations', value: stats?.orgs || 0, icon: Building2, color: 'text-violet-500' }, { label: 'Pending Reports', value: stats?.pendingReports || 0, icon: AlertTriangle, color: 'text-red-500' }, { label: 'Posts', value: stats?.posts || 0, icon: FileText, color: 'text-sky-500' }];

  return (<div className="space-y-6"><div className="flex items-center justify-between"><h1 className="text-2xl font-bold tracking-tight"><Shield className="h-6 w-6 inline mr-2 text-primary" />Admin Panel</h1><Badge variant="default">Admin</Badge></div>
