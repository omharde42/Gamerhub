  ...
  return (<div className="max-w-3xl mx-auto space-y-6"><h1 className="text-2xl font-bold">Settings</h1>
    <Tabs defaultValue="profile"><TabsList><TabsTrigger value="profile"><User className="h-4 w-4 mr-1" />Profile</TabsTrigger><TabsTrigger value="gaming"><Gamepad2 className="h-4 w-4 mr-1" />Gaming</TabsTrigger><TabsTrigger value="social"><Shield className="h-4 w-4 mr-1" />Social</TabsTrigger><TabsTrigger value="notifications"><Bell className="h-4 w-4 mr-1" />Notifications</TabsTrigger></TabsList>
      ...
      <TabsContent value="gaming"><Card className="glass-card"><CardContent className="p-6 space-y-4">
        <div><Label>Main Games</Label><div className="flex gap-2 mt-1">{profile.mainGames.map((g, i) => (<Badge key={i} variant="secondary" className="gap-1">{g}<button onClick={() => setProfile({ ...profile, mainGames: profile.mainGames.filter((_, j) => j !== i) })}><X className="h-3 w-3" /></button></Badge>))}</div><div className="flex gap-2 mt-2"><Select value={newGame} onValueChange={setNewGame}><SelectTrigger><SelectValue placeholder="Add game" /></SelectTrigger><SelectContent>{GAMES.filter(g => !profile.mainGames.includes(g)).map((g) => (<SelectItem key={g} value={g}>{g}</SelectItem>))}</SelectContent></Select><Button variant="outline" size="sm" onClick={addGame}>Add</Button></div></div>
        <div><Label>Languages</Label><div className="flex gap-2 mt-1">{profile.languages.map((l, i) => (<Badge key={i} variant="outline" className="gap-1">{l}<button onClick={() => setProfile({ ...profile, languages: profile.languages.filter((_, j) => j !== i) })}><X className="h-3 w-3" /></button></Badge>))}</div><div className="flex gap-2 mt-2"><Select value={newLang} onValueChange={setNewLang}><SelectTrigger><SelectValue placeholder="Add language" /></SelectTrigger><SelectContent>{LANGUAGES.filter(l => !profile.languages.includes(l)).map((l) => (<SelectItem key={l} value={l}>{l}</SelectItem>))}</SelectContent></Select><Button variant="outline" size="sm" onClick={addLang}>Add</Button></div></div>
