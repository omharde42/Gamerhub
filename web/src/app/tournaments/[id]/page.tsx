...
export default function TournamentDetailPage() {
  ...
  return (<div className="max-w-4xl mx-auto space-y-6">
    <PageBackButton fallbackHref="/tournaments" className="-mb-2" />
    ...
    {tournament.matches?.length > 0 && (<Card className="glass-card"><CardHeader><CardTitle>Brackets</CardTitle></CardHeader><CardContent><div className="space-y-2">{tournament.matches.map((match: any, i: number) => (<div key={i} className="flex flex-col sm:flex-row sm:items-center justify-between p-3 rounded-lg border border-border gap-2"><div className="flex flex-wrap items-center gap-2 sm:gap-4"><Badge variant="outline">R{match.round}</Badge><span className="font-medium text-sm truncate max-w-[100px] sm:max-w-none">{match.team1?.team?.name || 'TBD'}</span><Swords className="h-4 w-4 text-muted-foreground shrink-0" /><span className="font-medium text-sm truncate max-w-[100px] sm:max-w-none">{match.team2?.team?.name || 'TBD'}</span></div><Badge variant={match.status === 'COMPLETED' ? 'success' : 'secondary'} className="w-fit">{match.status}</Badge></div>))}</div></CardContent></Card>)}
  </div>);
}
